...
export default async function main(client: GenLayerClient<GenLayerChain>) {
  const filePath = path.resolve(process.cwd(), "contracts/escrivan.py");
  try {
    const contractCode = new Uint8Array(readFileSync(filePath));
    await client.initializeConsensusSmartContract();

    // Escrivan's constructor takes no arguments — there is no protocol owner.
    ...
    console.log(`Escrivan deployed at: ${addr}`);
